...
import chatViewCss from './components/chat-view.css';

window.Vaadin = window.Vaadin || {};
window.Vaadin.Flow = window.Vaadin.Flow || {};
window.Vaadin.Flow['_vaadintheme_myapp_globalCss'] = window.Vaadin.Flow['_vaadintheme_myapp_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_myapp_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_myapp_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_myapp_globalCss'].push(target);
  }
  if (!document['_vaadintheme_myapp_componentCss']) {
    registerStyles(
      'chat-view',
      css`
        ${unsafeCSS(chatViewCss.toString())}
      `
    );
